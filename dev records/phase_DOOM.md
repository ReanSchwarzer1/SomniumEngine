...
| **J — bandwidth/formats/allocations** | **Complete.** wgpu's internal counters are on in every build and sampled per frame. Textures, views, bind groups and samplers do not move across 300 steady-state frames on either map; the one buffer that oscillates is `(wgpu internal) Staging`. Inventory: 1901.7 MiB allocated of 2368.0 MiB reserved, largest row 384 MiB of fixed geometry pools at ~3% occupancy. | Done. See [`DOOM-J.md`](phase%20DOOM/DOOM-J.md). No format changed — the census says per-pixel cost, not bandwidth. There is still no bytes-per-frame counter, and that is stated as a gap rather than closed over. |
...
The remaining dependency order is **K → L → M**. D, H, I and J are complete;
