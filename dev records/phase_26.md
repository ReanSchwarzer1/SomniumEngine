...
> **Successor phase planned:** Phase 27 — Hades
> ([`phase_27.md`](phase_27.md), planned 2026-08-18, not started) rebuilds the
> `somnium_ui` **paint layer** — an instanced SDF primitive pipeline (radius,
> antialiasing, gradients, real shadows, glow), text-quality work, an animation
> driver, token expansion plus a light theme, identity completion, and the
> project-picker / Content-Browser surfaces. It also evaluates and **rejects**
> replacing `somnium_ui` with Tauri, GPUI, Slint, or Iced/libcosmic
> (`phase_27.md` §3). On its completion, `phase_27.md` §16 rewrites this
> file's status block and closes 26-H and 26-D2. Until then this block stands.
