...
2026-09-09: [PERSONA-H's first implementation pass](PERSONA-H.md) is in tree on base `b58e788`: shared hover/press/toggle policy, bounded button and checkbox feedback, node cleanup and idle/interaction regressions. H was chosen before I under §8.1. Both token sheets are `0.4.0-persona`; this version currently adds interaction metrics only, not I's rendering features.

A second H pass implements anchored Popup entry/exit with immediate input and paint-only close, using scoped transforms for both paint streams and clipping. Its 788-test suite is green; see the H record for exact scope.

Full H is still open: centered popup/modal/drawer travel, sibling-menu switching, focus/selection/tab, groups and floating-window choreography, plus remaining curve/spring/stagger policy work and native acceptance. I–L and G remain open. The broader QoL backlog remains open. A [screenshot-feedback pass](PERSONA-QoL.md#expansion-feedback--2026-09-09) delivers bounded scrolling Create/View menus, control alignment and the user-requested viewport texel-density mode; this does not mark K complete. Existing user preferences for flat dark panels and immediate Content hover take precedence over conflicting appearance suggestions in this brief.
