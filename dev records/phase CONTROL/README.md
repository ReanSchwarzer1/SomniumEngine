...
| …and `S` still lagged 2–3 s | bare `S` is bound to the Scale tool, so a shortcut dispatcher ate the press; the `!repeat` guard meant OS key-repeat then fell through, which is why it started moving exactly when auto-repeat did | `viewport_camera_active` latch from right-press to release, cleared on a release anywhere and on window focus loss |
| …and it *still* did not work | there are **two** shortcut dispatchers. The engine's own (`app.rs` §1) runs before the UI is consulted and `return`s, so the first guard — in the UI dispatcher — was downstream of the damage | `shortcut_action_for(code, modifiers, game_owns_keyboard)`, extracted and tested, filtering unmodified chords. `Ctrl+S` still saves |
| …and again in Play mode | the guard keyed on the fly-cam latch, and a walking character is not flying the viewport | the predicate is now "the game owns the keyboard" — flying **or** a play session. Transport controls have no unmodified binding, and a test keeps it that way |
