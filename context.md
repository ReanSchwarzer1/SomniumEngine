...
| 24P | 🟡 Started | **Software fallback: mesh SDFs + global distance-field clipmap.** CPU AABB SDF uploaded into the 64³ volume alpha, then cone-traced in shading. Default off (`SOMNIUM_MESH_SDF=1`). Shares volume alpha with 24M — leave World Cache off while testing SDF. Packed mesh SDF bake still to follow. |
| 24Q | 🟡 Started | **Baked light probes.** Env-cube fallback into the world cache when GI misses. Default off (`SOMNIUM_PROBES=1` or **Probes**). Full SH irradiance volumes still to follow. |
| 24R | 🟡 Started | **Area lights (LTC).** `LightType::Rect` with polygon irradiance in `shading.wgsl`. Create → **Area Light**; Half W / Half H in Details. Disc/tube still to follow. |
...
| 25C | 🟡 Started | **CDLOD vertex morphing.** Packed in instance `_padding`; **LOD Morph** + **Morph** on the terrain inspector (default on, start 0.7). Reference: `CDLOD-master/source/BasicCDLOD/Shaders/CDLODTerrain.vsh`. |
