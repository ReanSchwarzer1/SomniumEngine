...
| 24W | ⬜ Planned | **Water in physical units.** The water shader predates 24A and still does its own lighting against what used to be an arbitrary sun multiplier, so in direct sun it blows out hard. Everything else in the renderer has been moved onto lux; this is the last holdout. Also wants the 24C atmosphere for its reflection instead of the planar approximation. |
| 24X | ⬜ Planned | **Screen-space shadows (Bend Studio).** A short screen-space ray march that recovers contact shadows a shadow map cannot resolve — the dark line where a trunk meets soil, a leaf on the leaf below it. Reference: Spartan's `bend_sss.hlsl`, which ports Bend's *Sampling-Free Screen-Space Shadows*. Complements 24H rather than replacing it. |
| 24Y | ⬜ Planned | **Colour grading LUTs.** A 3-D grading LUT applied after tone mapping, plus blending between two of them. This is the control an artist actually reaches for to set a film look, and no amount of exposure or tone-mapper choice substitutes. Reference: O3DE's `LutGeneration.azsl`, `BlendColorGradingLuts.azsl`, `AcesOutputTransformLut.azsl`. |
| 24Z | ⬜ Planned | **Lens realism: depth of field, motion blur, film grain, dithering.** Depth of field falls out of 24A almost for free — the physical camera already carries an aperture, and that aperture is exactly what sets the circle of confusion. Dithering matters more than it sounds: without it, smooth dark gradients band visibly once exposure is physical. Reference: Spartan's `depth_of_field.hlsl`, `motion_blur.hlsl`, `film_grain.hlsl`, `dithering.hlsl`. |
