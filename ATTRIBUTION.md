...
| `somnium_core/src/context.rs`, `app.rs`; `somnium_ui/src/lib.rs` (IV-I) | Unreal viewport transport convention; Somnium's fixed-step simulation clock, native-wgpu button routing, pause/freeze, and deterministic stop/reset behavior are original |
...
| `somnium_renderer/src/shaders/water.wgsl` (IV-I) | Wicked ripple/pass organization and Eidos shoreline-SDF research; original analytic Kelvin wake, prop wash, filtered shoreline coverage, breaker foam, and rotated multi-scale material detail |
...
| Wake and shoreline foam | **Wicked Engine** `wiScene.cpp`, `wiRenderPath3D.cpp`, and its ocean shaders (MIT, Turánszki János) were inspected for ripple injection and render-pass placement. Eidos Montréal's *From Shore to Horizon* informed SDF-scaled coastal bands. Somnium's analytic Kelvin-angle arms, prop wash, filtered coverage, metre-scaled SDF band, three-frequency normal stack, and WGSL integration are original. |
| Play/Pause/Stop | Unreal Engine's viewport transport convention informed the three-control editor UX. Somnium's `SimulationClock`, fixed accumulator, event routing, explicit pause/freeze, and reset semantics are original Rust implementation. Environmental preview runs in both Editing and Playing; Pause is the intentional freeze control. No Unreal source was copied. |
