...
impl UiPass {
    pub fn new(
        ...
        let vtx_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("UI Vertices"), size: INIT_VTX_CAP,
            usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let idx_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("UI Indices"), size: INIT_IDX_CAP,
    ...
    pub fn prepare(
        ...
        let proj = Mat4::orthographic_rh(
            0.0, surface_w as f32,
            surface_h as f32, 0.0,
            0.0, 1.0,
        );
        queue.write_buffer(&self.ortho_buf, 0, bytemuck::bytes_of(&proj.to_cols_array()));
        ...
        if draw_ctx.font_atlas.dirty {
            ...
            self.bg1_atlas = Self::make_bg1(
                device, &self.bg1_layout, &self.atlas_view, &self.sampler,
            );
        }
        ...
        if !draw_ctx.vertices.is_empty() {
            ...
            if needed > self.vtx_capacity {
                ...
                self.vtx_buf = device.create_buffer(&wgpu::BufferDescriptor {
                    label: Some("UI Vertices"), size: self.vtx_capacity,
                    usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
                    mapped_at_creation: false,
                });
            }
        ...
        if !draw_ctx.indices.is_empty() {
            ...
            if needed > self.idx_capacity {
                ...
                self.idx_buf = device.create_buffer(&wgpu::BufferDescriptor {
                    label: Some("UI Indices"), size: self.idx_capacity,
                    usage: wgpu::BufferUsages::INDEX | wgpu::BufferUsages::COPY_DST,
                    mapped_at_creation: false,
                });
            }
