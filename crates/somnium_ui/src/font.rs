...
impl GlyphKey {
    pub fn new(ch: char, px: f32, font_id: u8) -> Self {
        Self { codepoint: ch as u32, px_bits: px.to_bits(), font_id }
    }
}
...
impl FontAtlas {
    ...
    pub fn has_fonts(&self) -> bool { !self.fonts.is_empty() }
    ...
    pub fn measure_text(&self, text: &str, px: f32, font_id: u8) -> Vec2 {
        ...
        let line_h = font.horizontal_line_metrics(px)
            .map(|m| m.ascent - m.descent)
            .unwrap_or(px);
        let total_w: f32 = text.chars()
    ...
    pub fn get_or_rasterize(&mut self, ch: char, px: f32, font_id: u8) -> Option<GlyphInfo> {
        ...
        if metrics.width == 0 || metrics.height == 0 {
            let info = GlyphInfo {
                uv_min: [0.0; 2], uv_max: [0.0; 2],
                xmin: metrics.xmin as f32, ymin: metrics.ymin as f32,
                px_w: 0.0, px_h: 0.0,
