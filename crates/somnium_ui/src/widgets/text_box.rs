...
impl Control for TextBox {
    ...
    fn draw(&self, widget: &Widget, ctx: &mut DrawingContext) {
        ...
        let border_col = if self.focused { [100, 140, 200, 255] } else { [70, 70, 70, 255] };
        ...
        if self.focused {
            let advance = ctx.font_atlas.measure_text(&self.text, self.px, self.font_id).x;
        ...
        if let Some(wmsg) = msg.data::<WidgetMessage>() {
            match wmsg.clone() {
                ...
                WidgetMessage::Unfocus => {
                    self.focused = false;
                    emit.push(TextBoxMessage::text_commit(widget.handle, self.text.clone()));
                    msg.handled = true;
                }
                WidgetMessage::Text(s) => {
                    if self.focused {
                        ...
                        widget.invalidate_layout();
                        emit.push(TextBoxMessage::text_changed(widget.handle, self.text.clone()));
                        msg.handled = true;
                    }
                }
...
impl TextBoxBuilder {
    ...
    pub fn with_color(mut self, c: [u8; 4]) -> Self { self.color = c; self }

    pub fn build(self) -> UiNode {
        UiNode::new(self.widget.build(), Box::new(TextBox {
            text:    self.text,
            px:      self.px,
            color:   self.color,
