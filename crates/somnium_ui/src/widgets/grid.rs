...
impl GridDimension {
    pub fn strict(desired_size: f32) -> Self {
        Self { size_mode: SizeMode::Strict, desired_size, actual_size: desired_size, ..Default::default() }
    }
    pub fn auto() -> Self {
        Self { size_mode: SizeMode::Auto, ..Default::default() }
    }
    pub fn stretch() -> Self {
        Self { size_mode: SizeMode::Stretch, ..Default::default() }
    }
...
fn group_index(row: SizeMode, col: SizeMode) -> usize {
    match (row, col) {
        ...
        (SizeMode::Strict,  SizeMode::Stretch) |
        (SizeMode::Auto,    SizeMode::Stretch) => 2,
...
fn count_stretch(dims: &[GridDimension]) -> usize {
    dims.iter().filter(|d| d.size_mode == SizeMode::Stretch).count()
}
...
fn arrange_dims(dims: &mut [GridDimension], final_size: f32) {
    ...
    let avg = if stretch_n > 0 { (final_size - preset) / stretch_n as f32 } else { 0.0 };
...
impl Grid {
    fn calc_needed_measurements(&self, widget: &Widget, ctx: &mut LayoutCtx) {
        ...
        let positions: Vec<(usize, usize)> = widget.children.iter()
            ...
            .collect();
        {
            ...
            for (ri, ci) in positions {
                if let Some(c) = cols.get_mut(ci) {
                    if c.size_mode == SizeMode::Auto { c.unmeasured_node_count += 1; }
                }
                if let Some(r) = rows.get_mut(ri) {
                    if r.size_mode == SizeMode::Auto { r.unmeasured_node_count += 1; }
                }
            }
        }
    }
    ...
    fn initialize_measure(&self, widget: &Widget, ctx: &mut LayoutCtx) {
        ...
        let positions: Vec<(NodeHandle, usize, usize)> = widget.children.iter()
        ...
        for (ci, col) in columns.iter().enumerate() {
            for (ri, row) in rows.iter().enumerate() {
                ...
                let nodes: Vec<NodeHandle> = positions.iter()
                    .filter(|(_, r, c)| *r == ri && *c == ci)
                    .map(|(h, _, _)| *h)
                    .collect();
                cells.push(Cell { nodes, row_index: ri, column_index: ci });
            }
        }
    }
    ...
    fn measure_cell_node(
        ...
        let constraint = {
            ...
            let Some(row) = rows.get(ri) else { return; };
            let Some(col) = cols.get(ci) else { return; };
            Vec2::new(choose_constraint(col, avail.x), choose_constraint(row, avail.y))
        };
        ...
        let mut rows = self.rows.borrow_mut();
        let mut cols = self.columns.borrow_mut();
        let Some(row) = rows.get_mut(ri) else { return; };
        let Some(col) = cols.get_mut(ci) else { return; };
    ...
    fn measure_group(&self, group: &[usize], ctx: &mut LayoutCtx, avail: Vec2) {
        ...
        for n in nodes { self.measure_cell_node(n, ctx, avail, true, true); }
    }
    ...
    fn measure_group_width(&self, group: &[usize], ctx: &mut LayoutCtx, avail: Vec2) {
        ...
        for n in nodes { self.measure_cell_node(n, ctx, avail, true, false); }
    }
    ...
    fn measure_group_height(&self, group: &[usize], ctx: &mut LayoutCtx, avail: Vec2) {
        ...
        for n in nodes { self.measure_cell_node(n, ctx, avail, false, true); }
    }
}
...
impl Control for Grid {
    fn measure_override(&self, widget: &Widget, ctx: &mut LayoutCtx, available: Vec2) -> Vec2 {
        ...
        let (g0, g1, g2, g3) = {
            ...
            (groups[0].clone(), groups[1].clone(), groups[2].clone(), groups[3].clone())
        };
    ...
    fn arrange_override(&self, widget: &Widget, ctx: &mut LayoutCtx, final_size: Vec2) -> Vec2 {
        if self.columns.borrow().is_empty() || self.rows.borrow().is_empty() {
            ...
            for &ch in &widget.children { ctx.arrange_child(ch, rect); }
            return final_size;
        }
        ...
        for &ch in &widget.children {
            ...
            if let (Some(col), Some(row)) = (cols.get(ci), rows.get(ri)) {
                ctx.arrange_child(ch, Rect::new(ox + col.location, oy + row.location, col.actual_size, row.actual_size));
            }
        }
        final_size
    }
...
impl GridBuilder {
    pub fn new(widget: WidgetBuilder) -> Self {
        Self { widget, rows: Vec::new(), columns: Vec::new(), draw_border: false, border_thickness: 1.0 }
    }

    pub fn add_row(mut self, row: Row) -> Self { self.rows.push(row); self }
    pub fn add_column(mut self, col: Column) -> Self { self.columns.push(col); self }
    pub fn with_draw_border(mut self, v: bool) -> Self { self.draw_border = v; self }
