...
| Flutter / Flutter Hz | Bend the sprite above its fixed base. Zero disables bending; use restrained values for flame. Works with atlases and pauses with simulation. |
| Staged reflection | Reflect live sprites in the active staged mirror alcove. Age and emission run once; the reflected copy reverses its texture. Intended for vertical mirrors. |
...
flowchart LR
  ...
  Sim --> Mirror[Optional staged reflection copy]
  Mirror --> View
  Pool --> View
